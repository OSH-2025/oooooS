...
>如何使用Cargo来调用生成的静态库，请看[使用Cargo在Rust中调用C](#Rust调用C)
1. **文件结构：**
    ```
    .
...
## 3. 总结
经过对Cargo、SCons和Meson的详细分析，我们选择使用SCons构建C代码，使用Cargo构建Rust相关代码。这种组合方案充分利用了SCons在嵌入式开发中的成熟支持和Cargo在Rust代码构建中的强大功能，能够有效满足我们项目的需求，同时减少构建过程中的复杂度和潜在问题。
