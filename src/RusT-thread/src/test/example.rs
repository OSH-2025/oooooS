...
pub fn run_comprehensive_demo() {
    crate::test::comprehensive_example::run_comprehensive_demo();
}

/// 运行多级反馈队列调度演示
pub fn run_mfq_demo() {
    crate::test::comprehensive_example::run_mfq_demo();
}
...
pub fn run_all_demos() {
    ...
    // 等待一段时间后运行全面演示
    rt_thread_sleep(rt_thread_self().unwrap(), 15000);
    
    hprintln!("2. 运行全面功能演示...");
    run_comprehensive_demo();
    
    // 等待一段时间后运行MFQ演示
    rt_thread_sleep(rt_thread_self().unwrap(), 25000);
    
    hprintln!("3. 运行多级反馈队列调度演示...");
    run_mfq_demo();
