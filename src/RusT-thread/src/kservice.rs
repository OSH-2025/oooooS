...
/// 使用示例
/// 
/// static DATA: FreeCell<u32> = FreeCell::new(0);
/// 
/// 安全地访问和修改
/// let mut data = DATA.exclusive_access();
/// *data += 1;
/// 
pub struct FreeCell<T> {
    /// inner data
    inner: RefCell<T>,
}

unsafe impl<T> Sync for FreeCell<T> {}

pub struct FreeCellRefMut<'a, T> {
...
impl<T> FreeCell<T> {
    ...
    pub fn exclusive_access(&self) -> FreeCellRefMut<'_, T> {
        // hprintln!("exclusive_access");
        FreeCellRefMut {
...
impl<'a, T> Drop for FreeCellRefMut<'a, T> {
    fn drop(&mut self) {
        // hprintln!("drop");
        self.inner = None;
    }
...
impl<'a, T> Deref for FreeCellRefMut<'a, T> {
    type Target = T;
...
impl<'a, T> DerefMut for FreeCellRefMut<'a, T> {
