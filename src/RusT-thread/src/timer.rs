...
impl Drop for RtTimer {
    fn drop(&mut self) {
        // This code runs automatically when RtTimer is dropped (last Rc is dropped)
...
// 定时器句柄类型
pub type TimerHandle = Rc<RefCell<RtTimer>>;

// 单线程环境下的全局定时器数组，只能通过本文件的接口操作
static mut TIMERS: Option<RefCell<Vec<TimerHandle>>> = None;

/// 初始化定时器系统（可选，根据需要调用）
pub fn timer_system_init() {
    unsafe {
        TIMERS = Some(RefCell::new(Vec::new()));
    }
}
...
pub fn rt_timer_start(timer: TimerHandle) {
    ...
    timer.borrow_mut().parent.flag |= RT_TIMER_FLAG_ACTIVATED;
    timer.borrow_mut().timeout_tick = timer.borrow().init_tick + rt_tick_get();
    let timeout_tick = timer.borrow().timeout_tick;
    unsafe {
        if let Some(ref timers) = TIMERS {
            let mut timers = timers.borrow_mut();
            let pos = timers.binary_search_by(|probe| probe.borrow().timeout_tick.cmp(&timeout_tick))
                .unwrap_or_else(|e| e);
...
pub fn rt_timer_stop(timer: &TimerHandle) {
    ...
    unsafe {
        if let Some(ref timers) = TIMERS {
            let mut timers = timers.borrow_mut();
            if let Some(pos) = timers.iter().position(|t| Rc::ptr_eq(t, timer)) {
                let timer_to_destroy = timers.remove(pos); // 移除 Rc 句柄
                // 当 timer_to_destroy 离开作用域时，如果它是最后一个 Rc，Drop trait for RtTimer will be called.
            }
        }
    }
    rt_hw_interrupt_enable(level);
...
pub fn rt_timer_check() {
    ...
    unsafe {
        if let Some(ref timers) = TIMERS {
            ...
            let pos = timers.binary_search_by(|timer| {
                let t = timer.borrow();
                if (current_tick.wrapping_sub(t.timeout_tick)) < 0x7fffffff {
    ...
    for timer in expired_timers {
        timer.borrow_mut().trigger_timeout();
